...
for TYP in (:RaggedMatrix,:Matrix,:BandedMatrix,
    ...
    @eval convert(::Type{$TYP},P::SubOperator{T,PP,Tuple{UnitBlockRange,UnitBlockRange}}) where {T,PP<:PlusOperator} =
...
for (TYP,ZERS) in ((:BandedMatrix,:bzeros),(:Matrix,:zeros),
    ...
    @eval begin
        # avoid ambiugity
        convert(::Type{$TYP},
                     S::SubOperator{T,OP,Tuple{UnitBlockRange,UnitBlockRange}}) where {T,OP<:ConstantTimesOperator} =
            convert_axpy!($TYP,S)
...
for (STyp,Zer) in ((:BandedBlockBandedMatrix,:bbbzeros),
                    (:BlockBandedMatrix,:bbzeros))
    @eval function convert(::Type{$STyp},
   S::SubOperator{T,TO,Tuple{UnitBlockRange,UnitBlockRange}}) where {T,TO<:TimesOperator}
        P=parent(S)
        KR,JR=parentindexes(S)
